...
const s = StyleSheet.create({
  ...
  sidebar: { width: 178, backgroundColor: SIDEBAR_BG, padding: '36 16 36 18' },
  ...
  mSectionTitle: {
    ...
    color: SIDEBAR_BG, textTransform: 'uppercase', marginBottom: 2, marginTop: 14,
  },
  mDivider: { borderBottomWidth: 1, borderBottomColor: SIDEBAR_BG, marginBottom: 7 },
...
export function ModernTemplate({ data, labels }: { data: ResumeData; labels: Labels }) {
    ...
    <Document>
      <Page size={data.pageSize || 'LETTER'} style={s.page}>
        ...
        <View style={s.main}>
          {data.summary && (
            <View>
              <Text style={s.mSectionTitle}>{labels.summary}</Text>
          ...
          {experience.length > 0 && (
            <View>
              <Text style={s.mSectionTitle}>{labels.experience}</Text>
              <View style={s.mDivider} />
              {experience.map((exp) => {
          ...
          {data.projects && data.projects.length > 0 && (
            <View>
              <Text style={[s.mSectionTitle, { marginTop: 14 }]}>{labels.projects}</Text>
              <View style={s.mDivider} />
          ...
          {education.length > 0 && (
            <View>
              <Text style={[s.mSectionTitle, { marginTop: experience.length > 0 ? 14 : 0 }]}>{labels.education}</Text>
              <View style={s.mDivider} />
              {education.map((edu) => {
          ...
          {data.certifications && data.certifications.length > 0 && (
            <View>
              <Text style={[s.mSectionTitle, { marginTop: 14 }]}>{labels.certifications}</Text>
              <View style={s.mDivider} />
              {data.certifications.map((cert) => (
                <View key={cert.id} style={{ marginBottom: 5, flexDirection: 'row', justifyContent: 'space-between' }}>
                  <Text style={{ fontSize: 9, fontFamily: SANS, fontWeight: 700, color: '#1a2744' }}>{cert.name}{cert.issuer ? ` · ${cert.issuer}` : ''}</Text>
